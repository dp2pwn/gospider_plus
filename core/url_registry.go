...
// DuplicateRequest canonicalizes method+URL+body and tracks uniqueness.
...
func canonicalRequestKey(method, rawURL, body string) string {
	...
	parsed.Path = normalizePathComponent(parsed.Path)

	if parsed.RawQuery != "" {
		parsed.RawQuery = normalizeQuery(parsed.RawQuery)
	}

	canonicalURL := NormalizeDisplayURL(parsed.String())
